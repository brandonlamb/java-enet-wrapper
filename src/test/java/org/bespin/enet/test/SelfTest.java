...
public class SelfTest {
  ...
  private static Queue<Event> eventQueue1 = new LinkedBlockingQueue<>(100);
  private static Queue<Event> eventQueue2 = new LinkedBlockingDeque<>(100);
  ...
  public static void main(String[] args) throws EnetException, UnknownHostException, InterruptedException {
    ...
    Thread serviceThread1 = new Thread(() -> {
      for (; ; ) {
        try {
          ...
          System.out.printf(
            "host 1 event; type: %s, peer: %s, data: %d, channel: %d, packet: %s%n",
            event.type(),
            event.peer(),
            event.data(),
            event.channelID(),
    ...
    Thread serviceThread2 = new Thread(() -> {
      for (; ; ) {
        try {
